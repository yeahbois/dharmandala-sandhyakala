<x-layout title="Order Summary | J V L Y N">
    <div class="w-full max-w-4xl mx-auto py-12 px-4 sm:px-6">
        <div class="flex flex-col items-center text-center mb-10">
            <div
                class="w-20 h-20 bg-secondary/20 text-secondary border border-secondary/30 flex items-center justify-center shadow-sm mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="bg-surface border border-outline/10 p-6 space-y-6">
                <h3 class="text-lg font-black uppercase tracking-tight text-on-surface border-b border-outline/10 pb-3">
                ...
                <div class="space-y-4">
                    ...
                    <div class="pt-4 border-t border-outline/10">
                        ...
                        <span
                            class="inline-block px-3 py-1 text-[10px] font-black uppercase tracking-widest {{ $order->order_status === 'paid' ? 'bg-secondary text-on-primary' : 'bg-primary text-on-primary' }}">
                            {{ $order->order_status }}
                        </span>
                    </div>
                </div>
        ...
        <div class="mt-8 bg-surface border border-outline/10 p-6 space-y-6">
            <h3 class="text-lg font-black uppercase tracking-tight text-on-surface border-b border-outline/10 pb-3">
                Entry Pass Details</h3>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs">
                    <thead>
                        <tr class="border-b border-outline/10">
                            <th class="py-3 uppercase tracking-widest font-black">Entry Pass ID</th>
                    ...
                    <tbody class="divide-y divide-outline/5">
                        @foreach($order->tickets as $ticket)
                            <tr>
                                ...
                                <td class="py-4 uppercase text-[10px] font-bold text-secondary">
                                    {{ $ticket->referral_code ?? '-' }}</td>
                                <td class="py-4 font-bold">IDR {{ number_format($ticket->price, 0, ',', '.') }}</td>
                                <td class="py-4">
                                    <span
                                        class="px-2 py-0.5 font-bold uppercase tracking-tighter text-[9px] {{ $ticket->ticket_status === 'pending_delivery' ? 'bg-secondary/10 text-secondary border border-secondary/20' : 'bg-primary/10 text-primary border border-primary/20' }}">
        ...
        <div class="mt-10 flex justify-center gap-4">
            <a href="/jvlyn"
                class="text-xs font-black uppercase tracking-widest bg-surface border border-outline/10 hover:bg-surface-variant/30 text-on-surface px-8 py-4 transition-colors">
                Back to Home
            </a>
        </div>
    </div>
