<x-layout title="Check Tickets | J V L Y N">
        ...
        <div class="bg-surface border border-outline/10 p-8 shadow-sm mb-10">
            <form action="{{ route('jvlyn.entry_pass.check.post') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Full Name</label>
                    ...
                    <div class="space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Email Address</label>
        ...
        @if(isset($searched))
            @if($orders->isEmpty())
                <div class="bg-brand-red/5 border border-brand-red/20 p-6 text-center">
                    <p class="text-sm font-bold text-brand-red uppercase tracking-widest">No orders found with these details.</p>
                </div>
            @else
                <div class="space-y-8">
                    @foreach($orders as $order)
                        <div class="bg-surface border border-outline/10 overflow-hidden">
                            <div class="p-6 border-b border-outline/10 bg-surface-variant/5 flex justify-between items-center">
                                <div>
                                    <h3 class="text-sm font-black uppercase tracking-widest text-on-surface">Order #{{ $order->id }}</h3>
                                    <p class="text-[10px] text-on-surface-variant uppercase mt-1">{{ $order->created_at->format('d M Y') }} • {{ $order->order_status }}</p>
                                </div>
