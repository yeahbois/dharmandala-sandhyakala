    ...
    <div class="w-full max-w-7xl mx-auto py-8 px-4 sm:px-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div class="bg-surface border border-outline/10 p-6 shadow-sm">
                <p class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant mb-2">Total Orders
                </p>
                <h3 class="text-3xl font-black text-on-surface">{{ $totalOrders }}</h3>
                <div class="mt-4 flex gap-2">
                    <span
                        class="text-[9px] font-bold uppercase tracking-tighter px-2 py-0.5 bg-secondary/10 text-secondary border border-secondary/20">Paid:
                        {{ $ordersByStatus['paid'] ?? 0 }}</span>
                    <span
                        class="text-[9px] font-bold uppercase tracking-tighter px-2 py-0.5 bg-primary/10 text-primary border border-primary/20">Pending:
                        {{ $ordersByStatus['pending'] ?? 0 }}</span>
                </div>
            </div>
            <div class="bg-surface border border-outline/10 p-6 shadow-sm">
                <p class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant mb-2">Entry Pass Sold
                </p>
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                    @foreach($ticketsByType as $type => $count)
                        <span
                            class="text-[9px] font-bold uppercase tracking-tighter px-2 py-0.5 bg-background border border-outline/10">{{ $type }}:
                            {{ $count }}</span>
                    @endforeach
                </div>
            </div>
            <div class="bg-surface border border-outline/10 p-6 shadow-sm">
                <p class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant mb-2">VIP Seats
                    Available</p>
                <h3 class="text-3xl font-black text-secondary" id="realtime-vip">{{ $availableVIP }}</h3>
                <p class="text-[9px] text-on-surface-variant mt-2 uppercase font-bold tracking-widest">Real-time
                    Syncing...</p>
            </div>
            <div class="bg-surface border border-outline/10 p-6 shadow-sm">
                <p class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant mb-2">Items in Cart
                </p>
                <h3 class="text-3xl font-black text-primary" id="realtime-carts">{{ $cartsCount }}</h3>
                <p class="text-[9px] text-on-surface-variant mt-2 uppercase font-bold tracking-widest">Active Sessions
                </p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div class="bg-surface border border-outline/10 p-6 shadow-sm border-l-4 border-l-secondary">
                <p class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant mb-2">Entry Pass Sent
                </p>
                <h3 class="text-3xl font-black text-secondary">{{ $ticketsSent }}/{{ $totalTickets }}</h3>
                <p class="text-[9px] text-on-surface-variant mt-2 uppercase font-bold tracking-widest">Email Delivered
                </p>
            </div>
            <div class="bg-surface border border-outline/10 p-6 shadow-sm border-l-4 border-l-primary">
                <p class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant mb-2">Entry Pass
                    Scanned</p>
            ...
            <div class="bg-surface border border-outline/10 p-6 shadow-sm border-l-4 border-l-yellow-500">
                ...
                <p class="text-[9px] text-on-surface-variant mt-2 uppercase font-bold tracking-widest">Waiting in Queue
                </p>
            </div>
            <div class="bg-surface border border-outline/10 p-6 shadow-sm border-l-4 border-l-brand-red">
                ...
                <p class="text-[9px] text-on-surface-variant mt-2 uppercase font-bold tracking-widest">Delivery Failures
                </p>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                @foreach(['Festival', 'VIP Seat', 'VIP Random'] as $cat)
                    <div class="p-4 border border-outline/10 bg-surface-variant/5">
                        <p class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant mb-4">{{ $cat }}
                        </p>
                        <div class="flex gap-2">
                            <form action="{{ route('jvlyn.panit.toggle-sale') }}" method="POST" class="flex-1">
                                ...
                                <button type="submit"
                                    class="w-full py-2 bg-brand-red text-white text-[9px] font-black uppercase tracking-widest hover:opacity-90 transition-all">Close
                                    Sale</button>
                            </form>
                            <form action="{{ route('jvlyn.panit.toggle-sale') }}" method="POST" class="flex-1">
                                @csrf
                                ...
                                <button type="submit"
                                    class="w-full py-2 bg-secondary text-white text-[9px] font-black uppercase tracking-widest hover:opacity-90 transition-all">Open
                                    Sale</button>
                            </form>
                        </div>
                    </div>
        ...
        <div class="bg-surface border border-outline/10 shadow-sm overflow-hidden">
            <div class="p-6 border-b border-outline/10 flex justify-between items-center">
                ...
                <button onclick="exportToCSV()"
                    class="text-[10px] font-black uppercase tracking-widest bg-background border border-outline/20 px-4 py-2 hover:bg-surface-variant/30 transition-all">Export
                    CSV</button>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs whitespace-nowrap">
                    <thead>
                        <tr class="bg-surface-variant/10 border-b border-outline/10">
                            ...
                            <th class="p-4 uppercase font-black tracking-widest">Entry Passes</th>
                                    ...
                                    <div class="flex flex-col gap-1">
                                        @foreach($order->tickets as $ticket)
                                            <div class="flex items-center gap-1">
                                                <span
                                                    class="text-[9px] font-mono font-bold px-1.5 py-0.5 bg-background border {{ $ticket->ticket_status === 'sent' ? 'border-green-500' : ($ticket->ticket_status === 'pending_delivery' ? 'border-yellow-500' : (in_array($ticket->ticket_status, ['failed', 'fail_order']) ? 'border-red-500' : 'border-black')) }} rounded-sm"
                                                    title="Price: IDR {{ number_format($ticket->price, 0) }} | Ref: {{ $ticket->referral_code ?? '-' }}">
                                                    {{ $ticket->ticket_id }} ({{ $ticket->ticket_type }}) - IDR
                                                    {{ number_format($ticket->price, 0, ',', '.') }}
                                                </span>
                                                <form action="{{ route('jvlyn.panit.ticket.delete', $ticket->id) }}"
                                                    method="POST" onsubmit="return confirm('Hapus tiket ini?')">
                                                    @csrf @method('DELETE')
                                                    <button type="submit"
                                                        class="text-[10px] text-brand-red hover:underline">Delete</button>
                                                </form>
                                            </div>
                                        @endforeach
                                    </div>
                                </td>
                                <td class="p-4 font-black text-primary">IDR {{ number_format($order->price, 0, ',', '.') }}
                                </td>
                                <td class="p-4">
                                    <span
                                        class="px-2 py-0.5 text-[9px] font-black uppercase tracking-widest border {{ $order->order_status === 'paid' ? 'bg-secondary/10 text-secondary border-secondary/20' : ($order->order_status === 'declined' ? 'bg-brand-red/10 text-brand-red border-brand-red/20' : 'bg-primary/10 text-primary border-primary/20') }}">
                                        {{ $order->order_status }}
                                    </span>
                                </td>
                                <td class="p-4">
                                    <div class="flex items-center gap-3">
                                        <a href="{{ route('jvlyn.summary', $order->id) }}"
                                            class="text-primary font-bold hover:underline">Details</a>
                                        <form action="{{ route('jvlyn.panit.order.delete', $order->id) }}" method="POST"
                                            onsubmit="return confirm('Hapus seluruh order ini?')">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="text-brand-red font-bold hover:underline">Delete
                                                Order</button>
                                        </form>
                                    </div>
                                </td>
